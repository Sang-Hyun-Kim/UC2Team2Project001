...
public:
	...
	shared_ptr<Skill> GetRandomActiveSkill(Character* owner);
	shared_ptr<Skill> GetRandomPassiveSkill(Character* owner);
	...
	// 지정해둔 타입으로 스킬을 생성합니다.
	shared_ptr<Skill> CreateSkillFromType(const type_index& skillType, Character* owner);
	...
	void RegisterSkill()
	{
		SkillFactory[typeid(T)] = [](Character* owner) -> shared_ptr<Skill> 
			{
			return make_shared<T>(owner);
			};
	}
