...
#pragma region 스킬 사용 예제
	...
	// 활성 스킬 추가
	//SkillManager::GetInstance().AddSkillToCharacter(player.get(), SkillType::ACTIVE);

	// 패시브 스킬 추가
	//SkillManager::GetInstance().AddSkillToCharacter(player.get(), SkillType::PASSIVE);


	int desiredSkillCount = 3;
	vector<type_index> uniqueSkillTypes = skillManager.GetUniqueRandomSkillTypes(player.get(), SkillType::ACTIVE, desiredSkillCount);

	std::cout << "획득한 스킬 유형 목록:\n";
	for (const auto& skillType : uniqueSkillTypes)
	{
		std::cout << " - " << skillType.name() << "\n";
	}


	Skill* createdSkill = skillManager.CreateSkillFromType(uniqueSkillTypes[1], player.get());

	if (createdSkill)
	{
		std::cout << "스킬 생성 성공: " << createdSkill->GetSkillData().skillName << std::endl;
	}
	else
	{
		std::cerr << "스킬 생성 실패\n";
	...
	// 중복되지 않은 활성 스킬 추가
	//skillManager.AddUniqueSkillToCharacter(player.get(), SkillType::ACTIVE);
